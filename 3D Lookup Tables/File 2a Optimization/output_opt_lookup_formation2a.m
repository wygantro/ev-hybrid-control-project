...
d=3;                                                %Sweep dimension length (sim runs=d^3)
    ...
    for j=1:d
        ...
        for k=1:d
            TA=torque_allocation(k);
                   ...
                   sim('output_opt_lookup_formation_sim2a.slx');
...
opt_output_power_location2a=A*1/k;
figure(1)
mesh(mtr_torque_Nm,mtr_speed_rad,opt_output_power_location2a)
...
opt_output_torque_location2a=B*1/k;
figure(2)
mesh(mtr_torque_Nm,mtr_speed_rad,opt_output_torque_location2a)
xlabel('Input Torque (Nm)'), ylabel('Input Motor Speed (rad/sec)'), zlabel('Output TA (%)')
title('Output Torque (Nm) Look Up')

%Vehicle output efficiency
[opt_output_efficiency,C]=max(efficiency,[],3);
opt_output_efficiency_location2a=C*1/k;
figure(3)
mesh(mtr_torque_Nm,mtr_speed_rad,opt_output_efficiency_location2a)
xlabel('Input Torque (Nm)'), ylabel('Input Motor Speed (rad/sec)'), zlabel('Output TA (%)')
title('Efficiency (miles/kWh) Look Up')

%sim('lookup_table_test.slx');

save('opt_output_power_location2a')
save('opt_output_torque_location2a')
save('opt_output_efficiency_location2a')
