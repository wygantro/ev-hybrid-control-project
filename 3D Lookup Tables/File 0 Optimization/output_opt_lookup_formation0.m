    ...
    for j=1:d
        ...
        for k=1:d
            TA=torque_allocation(k);
                   ...
                   sim('output_opt_lookup_formation_sim0.slx');
...
opt_output_power_location0=A*1/k;
figure(1)
mesh(mtr_torque_Nm,mtr_speed_rad,opt_output_power_location0)
...
opt_output_torque_location0=B*1/k;
figure(2)
mesh(mtr_torque_Nm,mtr_speed_rad,opt_output_torque_location0)
xlabel('Input Torque (Nm)'), ylabel('Input Motor Speed (rad/sec)'), zlabel('Output TA (%)')
title('Output Torque (Nm) Look Up')

%Vehicle output efficiency
[opt_output_efficiency,C]=max(efficiency,[],3);
opt_output_efficiency_location0=C*1/k;
figure(3)
mesh(mtr_torque_Nm,mtr_speed_rad,opt_output_efficiency_location0)
xlabel('Input Torque (Nm)'), ylabel('Input Motor Speed (rad/sec)'), zlabel('Output TA (%)')
title('Efficiency (miles/kWh) Look Up')

%sim('lookup_table_test.slx');

save('opt_output_power_location0')
save('opt_output_torque_location0')
save('opt_output_efficiency_location0')
